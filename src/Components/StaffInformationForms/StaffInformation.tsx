/* eslint-disable @typescript-eslint/block-spacing */
...
import RequestModal from '../ResponseComponent/RequestModal'
...
const StaffInformation: React.FC = () => {
  ...
  const [modalRequestOpen, setRequestOpen] = React.useState(false)
  ...
  const handleSubmit = async (e: FormEvent): Promise<void> => {
    ...
    try {
      ...
      setLoading(false)
      toast.error(
        `${Boolean(error?.response?.data?.message) || error?.message}`
      )
  ...
  return (
    <>
      <AppModal
        height="499px"
        width="466px"
        modalIsOpen={modalRequestOpen}
        setIsOpen={setRequestOpen}
      >
        <RequestModal action={() => {setRequestOpen(false)}}
 />
      </AppModal>
      ...
      <form className="" onSubmit={handleSubmit}>
        <div className="flex-1 w-full p-4 space-y-8 bg-white rounded-xl">
          {/* personal */}
          <div
            className="w-full p-4 border-2 border-dashed border-slate-400 rounded-xl "
            id="personal"
          >
            <h3 className="text-lg font-medium text-left uppercase text-neutral-500">
              Personal Details
            </h3>
