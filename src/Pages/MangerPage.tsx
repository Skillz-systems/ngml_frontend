...
const MangerPage: React.FC = () => {
    ...
    const fetchData = async (): Promise<void> => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/customer/eoi/password@gmail.com`,
          {
            headers: {
  ...
  return (
    <div
      // className="w-full h-full p-8 bg-white bg-opacity-50 rounded overflow-hidden flex flex-col justify-start items-start gap-8"
      style={{ overflowX: 'auto', height: '100vh' }}
    >

      <div className='m-5 bg-white/40 flex-1 p-5 overflow-x-hidden rounded-xl'>
      <div className="flex justify-between">
        <h3 className='text-left capitalize font-semibold text-3xl text-neutral-600'>Manager Page</h3>
        ...
        <div className="bg-white rounded-[20px] border border-gray-300">
          <div className="flex-col gap-8">
            <div className="w-full bg-white p-4">
              <div className="text-xl text-left font-semibold text-gray-700">
                EOI REQUEST
              </div>
            </div>

            <div
              className="border-t border-b border-gray-300 p-4 flex justify-between items-center"
              style={{ backgroundColor: '#F6FDEC' }}
            >
              ...
              <div
                className="p-2 bg-green-100 rounded-full flex items-center"
                style={{ backgroundColor: '#D2F69E' }}
              >
            ...
            <div className="pb-80 bg-white border border-solid border-gray-300 rounded-[20px] mr-3 ml-3 pt-[30px]">
              <div className="text-gray-400 text-16 text-left font-bold pb-4 ml-[30px]">
                REASON FOR REQUEST
              </div>
            ...
            <div
              style={{ backgroundColor: '#F6FDEC' }}
              className="w-full h-full mt-4 px-12 pt-8 pb-8 bg-white border-t border-solid border-gray-300 flex justify-between items-center"
            >
              <div className="flex justify-start items-center gap-10">
                <div className="text-gray-700 text-14 font-bold leading-14 break-words">
                  Approved By
                </div>
                <div className="text-gray-700 text-14 font-normal leading-14 break-words">
                  John Okolie
                </div>
              </div>
              <div className="flex justify-start items-center gap-10">
...
const NavItem: React.FC<{ text: string }> = ({ text }) => {
  return (
    <div style={{ whiteSpace: 'nowrap' }} className="text-left">
      <div className="font-semibold text-gray-600 text-left text-16 pb-2 pr-[50px]">
        {text}
      </div>
    </div>
  )
